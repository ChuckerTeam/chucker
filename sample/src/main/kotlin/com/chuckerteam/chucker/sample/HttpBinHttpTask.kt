...
class HttpBinHttpTask(
    ...
    private fun oneShotRequestBody() =
        object : RequestBody() {
    ...
    private interface Api {
        ...
        @GET("/stream/{lines}")
        fun stream(
            @Path("lines") lines: Int,
        ): Call<Any?>

        @GET("/stream-bytes/{bytes}")
        fun streamBytes(
            @Path("bytes") bytes: Int,
        ): Call<Any?>
        ...
        @POST("/post")
        fun postForm(
            @Field("key1") value1: String,
            @Field("key2") value2: String,
        ): Call<Any?>
